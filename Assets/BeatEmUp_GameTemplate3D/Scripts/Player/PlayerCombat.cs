...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject> {
	...
	public Transform weaponBone; //кость, от которой будет создано оружие
	...
	public float hitZRange = 2f; //диапозон атаки по Z
	private int attackNum = -1; //текущий номер комбо атаки
	[Space(5)]

	public DamageObject[] PunchCombo; //список комбо атаки руки
	public DamageObject[] KickCombo; //список комбо атаки ноги
	public DamageObject[] AnyCombo; //список из кастомных комбо
	...
	[Header("Settings")]
	public bool blockAttacksFromBehind = false; //блокировка атаки противника сзади
	...
	public float GroundAttackDistance = 1.5f; //расстояние от противника, на котором может быть осуществлена наземная атака
	...
	[Header ("Stats")]
	public DIRECTION currentDirection; //текущее направление
	...
	private bool continuePunchCombo; //true, если комбинацию ударов нужно продолжать
	private bool continueKickCombo; //true, если комбинацию ударов ногой нужно продолжать
	private float lastAttackTime = 0; //время, последней атаки
	[SerializeField]
	private bool targetHit; //true, если последний удар поразил цель
	...
	private void OnInputEvent(string action, BUTTONSTATE buttonState) {
		if (AttackStates.Contains (playerState.currentState) && !isDead) {
			...
			//сбросить комбо при переключении на другую цепочку комбо (настройка пользователя)
			if (resetComboChainOnChangeCombo && (action != lastAttackInput)){
				attackNum = -1;
			}

			//стандартный удар руки
			...
			//продолжать комбо ударов руки если "удар руки" была зажата
			if (action == "Punch" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState == UNITSTATE.PUNCH) 
			    && !continuePunchCombo && isGrounded) {
			...
			//продолжать комбо ударов ноги если "удар ноги" была зажата
		...
		if (action == "Punch" || action == "Kick" && buttonState == BUTTONSTATE.PRESS && playerState.currentState != UNITSTATE.PUNCH 
		    && playerState.currentState != UNITSTATE.KICK && isGrounded) {

			//перейдите к следующей атаке, если время находится внутри комбо-окна
			bool insideComboWindow = (lastAttack != null && (Time.time < (lastAttackTime + lastAttack.duration + lastAttack.comboResetTime)));
			if (insideComboWindow && !continuePunchCombo && (attackNum < AnyCombo.Length -1)) {
				attackNum += 1;
			} else {
				attackNum = 0;
			}

			if(AnyCombo[attackNum] != null && AnyCombo[attackNum].animTrigger.Length > 0) doAttack(AnyCombo[attackNum], AnyCombo[attackNum].playerState, AnyCombo[attackNum].GetAtackState());
			return;
		}
	}
	#endregion
